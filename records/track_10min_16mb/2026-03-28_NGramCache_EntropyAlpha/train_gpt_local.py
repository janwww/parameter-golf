...
torch.compile = lambda fn, **kwargs: fn  # local: disable torch.compile (Triton unavailable on Windows)
...
class Hyperparameters:
    ...
    # N-gram cache (eval-time, score-first, all variants)
    ...
    # alpha_mode: "entropy" (adaptive), "fixed" (constant), "logistic" (log-linear blend)
    ngram_alpha_mode = os.environ.get("NGRAM_ALPHA_MODE", "entropy")
    ngram_alpha_fixed = float(os.environ.get("NGRAM_ALPHA_FIXED", "0.5"))
    ngram_alpha_min = float(os.environ.get("NGRAM_ALPHA_MIN", "0.05"))
    ngram_alpha_max = float(os.environ.get("NGRAM_ALPHA_MAX", "0.60"))
    ngram_entropy_base = float(os.environ.get("NGRAM_ENTROPY_BASE", "4.0"))
    ngram_smoothing = float(os.environ.get("NGRAM_SMOOTHING", "0.01"))
    # logistic_mix=1: blend in log-prob space (log-linear) then renorm; 0: linear prob blend
    ngram_logistic_mix = bool(int(os.environ.get("NGRAM_LOGISTIC_MIX", "0")))
    # complementary training: down-weight bigram-easy tokens during training
    ngram_comp_train = bool(int(os.environ.get("NGRAM_COMP_TRAIN", "0")))
    ngram_comp_lambda = float(os.environ.get("NGRAM_COMP_LAMBDA", "0.3"))
    # local: load pre-trained weights from final_model.pt instead of training from scratch
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if _USE_FA3:
            ...
            # SDPA fallback for Ada Lovelace / Windows — q/k/v are (B,T,H,D); SDPA wants (B,H,T,D)
            q_t = q.transpose(1, 2)
            k_t = k.transpose(1, 2)
            v_t = v.transpose(1, 2)
            # GQA: expand KV heads to match Q heads (torch.compile needs explicit expansion)
            if k_t.size(1) != q_t.size(1):
                rep = q_t.size(1) // k_t.size(1)
...
class NGramCache:
    """Backward-looking n-gram count cache for score-first eval-time mixing.

    Maintains per-order count tables: counts[k][context_hash] = float32 array
    of length vocab_size+1, where the last element is the total count.
    Context hash is a polynomial rolling hash over the k-1 preceding tokens.

    All updates happen AFTER the corresponding token is scored (score-first).
    Each rank maintains its own independent cache (no cross-rank sync needed
    for legality; stats accumulate over the tokens each rank scores).
    """

    def __init__(self, max_order: int, min_order: int, vocab_size: int, smoothing: float = 0.01):
        self.max_order = max_order
        self.min_order = min_order
        self.vocab_size = vocab_size
        self.smoothing = smoothing
        # counts[k]: dict mapping int64 context hash → float32 array [vocab_size + 1]
        self.counts: list[dict] = [{} for _ in range(max_order + 1)]

    @staticmethod
    def _hash_ctx(tokens: np.ndarray) -> int:
        """Polynomial rolling hash over a short token array. Returns a Python int."""
        h = np.int64(0)
        for t in tokens:
            h = np.int64(h * np.int64(31337) + np.int64(int(t)))
        return int(h)

    def update_window(self, inp: np.ndarray, tgt: np.ndarray, s: int, wlen: int) -> None:
        """Update counts for all scored positions in a window.

        inp[pos] = token at position pos (the current/context token).
        tgt[pos] = next token = prediction target = inp[pos+1].
        Updates counts for positions s..wlen-1 using context = inp[pos-k+2..pos]
        (the k-1 tokens immediately preceding and including the current token).
        Called AFTER neural scoring to preserve score-first property.
        """
        vs = self.vocab_size
        counts = self.counts
        for pos in range(s, wlen):
            target = int(tgt[pos])
            max_k = min(self.max_order, pos + 1)
            for k in range(self.min_order, max_k + 1):
                ctx = inp[pos - k + 2 : pos + 1]  # k-1 context tokens
                h = self._hash_ctx(ctx)
                d = counts[k]
                if h not in d:
                    d[h] = np.zeros(vs + 1, dtype=np.float32)
                arr = d[h]
                arr[target] += 1.0
                arr[vs] += 1.0  # running total

    def query_positions(self, inp: np.ndarray, s: int, wlen: int) -> np.ndarray:
        """Return log-probs for each scored position using backoff.

        For position pos, tries orders max_order..min_order, uses the first
        that has a non-zero count entry. Falls back to uniform if none found.

        Returns float32 array of shape [wlen-s, vocab_size].
        Called BEFORE update_window for the same window (score-first).
        """
        n_pos = wlen - s
        vs = self.vocab_size
        log_uniform = float(-math.log(vs))
        result = np.full((n_pos, vs), log_uniform, dtype=np.float32)
        counts = self.counts
        smoothing = self.smoothing

        for j, pos in enumerate(range(s, wlen)):
            max_k = min(self.max_order, pos + 1)
            for k in range(max_k, self.min_order - 1, -1):
                ctx = inp[pos - k + 2 : pos + 1]
                h = self._hash_ctx(ctx)
                d = counts[k]
                if h in d:
                    arr = d[h]
                    total = arr[vs]
                    if total > 0.0:
                        probs = (arr[:vs] + smoothing) / (total + smoothing * vs)
                        result[j] = np.log(probs + 1e-40)
                        break
        return result
...
def eval_val_sliding_ttt(
    ...
    ngram_cache: "NGramCache | None" = None,
) -> tuple[float, float]:
    ...
    When ngram_cache is provided, neural log-probs are blended with backward-looking
    n-gram log-probs at each scored position (still score-first: query before update)."""
    ...
    ngram_info = (
        f" ngram=on order={args.ngram_min_order}-{args.ngram_max_order}"
        f" alpha_mode={args.ngram_alpha_mode}"
        f" logistic_mix={int(args.ngram_logistic_mix)}"
        if ngram_cache is not None else " ngram=off"
    )
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"freeze_blocks={args.ttt_freeze_blocks}{ngram_info}")
    ...
    alpha_sum = 0.0; alpha_sq_sum = 0.0; alpha_count = 0
    ...
    for ci in range(num_chunks):
        ...
        with torch.inference_mode():
            for bi in range(0, len(my_windows), batch_seqs):
                ...
                # Compute neural log-softmax once if n-gram blending is active
                # (shared across windows in this batch)
                if ngram_cache is not None:
                    log_probs_all = F.log_softmax(
                        logits.float(), dim=-1
                    ).cpu().numpy()  # [bsz, seq_len, vocab]
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    if ngram_cache is not None:
                        n_scored = wlen - s
                        inp_np = x_batch[i, :wlen].cpu().numpy().astype(np.int32)
                        tgt_np = y_batch[i, :wlen].cpu().numpy().astype(np.int32)

                        # Query n-gram BEFORE updating (score-first property)
                        ng_lp = ngram_cache.query_positions(inp_np, s, wlen)  # [n, vocab]
                        neural_lp = log_probs_all[i, s:wlen]                  # [n, vocab]

                        # Compute per-token entropy from neural distribution
                        H = -(np.exp(neural_lp) * neural_lp).sum(axis=-1)     # [n]

                        # Compute mixing weight alpha per position
                        am = args.ngram_alpha_mode
                        if am == "fixed":
                            alpha = np.full(n_scored, args.ngram_alpha_fixed, dtype=np.float32)
                        else:  # "entropy" (default) or fallback
                            # α = α_min + (α_max − α_min) · σ(2(H − H_0))
                            # High entropy (uncertain neural model) → high α → more n-gram weight
                            # Low entropy (confident neural model) → low α → trust neural
                            alpha = (args.ngram_alpha_min +
                                     (args.ngram_alpha_max - args.ngram_alpha_min) *
                                     (1.0 / (1.0 + np.exp(-2.0 * (H - args.ngram_entropy_base))))).astype(np.float32)

                        # Accumulate alpha stats for end-of-eval logging
                        alpha_sum += float(alpha.sum())
                        alpha_sq_sum += float((alpha ** 2).sum())
                        alpha_count += n_scored

                        # Blend distributions
                        if args.ngram_logistic_mix:
                            # Log-linear (logistic-domain) mix: interpolate in log-prob space, renorm
                            blended_lp = (1.0 - alpha[:, None]) * neural_lp + alpha[:, None] * ng_lp
                            # Subtract log-partition to renormalize
                            blended_lp -= np.log(np.exp(blended_lp).sum(axis=-1, keepdims=True) + 1e-40)
                        else:
                            # Linear probability mix: P = (1-α)P_neural + α P_ngram
                            blended_p = ((1.0 - alpha[:, None]) * np.exp(neural_lp) +
                                         alpha[:, None] * np.exp(ng_lp))
                            blended_lp = np.log(blended_p + 1e-40)

                        # NLL from blended distribution for target tokens
                        tgt_idx = tgt_np[s:wlen]
                        blended_nll_np = -blended_lp[np.arange(n_scored), tgt_idx]
                        scored_nll = torch.from_numpy(blended_nll_np).to(dtype=torch.float64, device=device)

                        # Update n-gram cache AFTER scoring (score-first)
                        ngram_cache.update_window(inp_np, tgt_np, s, wlen)
                    else:
                        scored_nll = nll[i, s:wlen].to(torch.float64)
    ...
    if ngram_cache is not None and alpha_count > 0:
        am = alpha_sum / alpha_count
        asd = math.sqrt(max(alpha_sq_sum / alpha_count - am ** 2, 0))
        log0(f"ngram:alpha_stats mean={am:.4f} std={asd:.4f} n={alpha_count}")
...
def main() -> None:
    ...
    enable_mem_efficient_sdp(not _USE_FA3)  # local: allow mem_efficient when FA3 unavailable
    enable_math_sdp(not _USE_FA3)           # local: allow math fallback when FA3 unavailable
    logfile = None
    if master_process:
    ...
    # local: optionally load pre-trained weights and skip training entirely
    if args.load_checkpoint:
        log0(f"load_checkpoint: loading weights from {args.load_checkpoint}")
        ckpt_sd = torch.load(args.load_checkpoint, map_location="cpu")
        # final_model.pt uses the banked layout; load with strict=False to handle minor key mismatches
        missing, unexpected = base_model.load_state_dict(ckpt_sd, strict=False)
        if missing:
            log0(f"load_checkpoint: missing keys: {missing[:5]}{'...' if len(missing) > 5 else ''}")
        if unexpected:
            log0(f"load_checkpoint: unexpected keys: {unexpected[:5]}{'...' if len(unexpected) > 5 else ''}")
        log0(f"load_checkpoint: done")
    ...
    step = 0
    if args.load_checkpoint:
        log0("load_checkpoint: skipping training loop, jumping to eval")
        args.iterations = 0
        # Reinitialize EMA state from loaded weights so EMA apply is a no-op
        ema_state = {name: t.detach().float().clone() for name, t in base_model.state_dict().items()}
    while True:
    ...
    # local: skip secondary stride=64 pass (saves ~2.3h; not needed for n-gram delta comparison)
    # Legal score-first TTT (PR #461 recipe)
    if args.ttt_enabled:
        # Optionally build a backward-looking n-gram cache (Engram-inspired, entropy-adaptive)
        ngram_cache: NGramCache | None = None
        if args.ngram_enabled:
            ...
            log0(
                f"ngram:init max_order={args.ngram_max_order} min_order={args.ngram_min_order} "
                f"alpha_mode={args.ngram_alpha_mode} alpha_min={args.ngram_alpha_min} "
                f"alpha_max={args.ngram_alpha_max} entropy_base={args.ngram_entropy_base} "
                f"smoothing={args.ngram_smoothing} logistic_mix={int(args.ngram_logistic_mix)}"
            )
        torch.cuda.synchronize()
        ...
        ttt_loss, ttt_bpb = eval_val_sliding_ttt(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, log0=log0, ngram_cache=ngram_cache,
        )
        torch.cuda.synchronize()
        ...
        if args.ngram_enabled:
            log0(f"ngram_delta bpb_delta:{ttt_bpb - sw_val_bpb:.6f} "
                 f"ttt_ngram:{ttt_bpb:.6f} sliding_baseline:{sw_val_bpb:.6f}")
    if distributed:
